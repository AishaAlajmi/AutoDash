// File: api/analyze.js
// Node serverless function for Vercel – robust body parsing + flexible payload shape (top-level or {payload})

export default async function handler(req, res) {
  try {
    // Read raw body (since req.body is undefined on Vercel Node by default)
    const chunks = [];
    ...
    // Accept either { payload: {...} } or a top-level Gemini payload {...}
    const payload = body?.payload ?? body;
    console.log("Received payload keys:", Object.keys(payload || {}));

    // Validate basic Gemini shape
    ...
    return res.status(200).send(text); // pass through JSON as-is
  } catch (err) {
